...
When Docker verification is expected to validate real bot behavior, do not stop at container health. After `docker compose up --build -d telegram-bot`, run `uv run anime-bot-ping` from the repository root with `TELEGRAM_E2E_BOT_USERNAME` and `TELEGRAM_E2E_SESSION_STRING`; the helper may reuse `TELEGRAM_API_ID` and `TELEGRAM_API_HASH` from `.env` when dedicated `TELEGRAM_E2E_API_ID` and `TELEGRAM_E2E_API_HASH` are not set.
